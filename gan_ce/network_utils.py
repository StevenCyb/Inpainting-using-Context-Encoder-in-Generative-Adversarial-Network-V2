...
def _conv2d(input, name, filters=64, kernel_size=5, strides=2, padding="same"):
    with tf.variable_scope(name):
        return conv2d(inputs=input, name=name, filters=filters, kernel_size=kernel_size, strides=strides, padding=padding)

def _conv2d_transpose(input, name, filters=64, kernel_size=5, strides=2, padding="same"):
    with tf.variable_scope(name):
